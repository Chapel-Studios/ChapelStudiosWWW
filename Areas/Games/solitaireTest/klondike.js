class KlondikeMove extends SolitaireMove {
    ...
    Complete () {
        ...
        if (this.Origin.ZoneType === "Hands") {
            let lastHand = NSJ.GetDeepestChild(`#${this.Origin.ZoneName} .hand`);
            if (lastHand.childNodes.length === 0) {
                lastHand.remove();
            }
        }
        else if (this.Origin.ZoneType === "Stack") {
            let BottomCard = NSJ.GetDeepestChild(`#${this.Origin.ZoneName} .playing-card`);
    ...
    UpdateMove = (mouseMoveEvent) => {
        ...
        if (currentMove) {
            if (currentMove.IsActive) {
                ...
                currentMove.CurrentDropZone = NSJ.GetParentID(target)
